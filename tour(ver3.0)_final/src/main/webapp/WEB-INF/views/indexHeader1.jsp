...
<body>
	<div id="menu1">
		<nav id="header1" class="navbar navbar-expand-sm navbar-light" style="width:80%; margin: auto;">
					...
					<ul class="navbar-nav">
						<li class="nav-item">
							<c:if test="${m_id == 'admin'}">
								<a class="nav-link" href="/uploadContentForm" id="nav-link">컨텐츠 등록하기</a>
							</c:if>
							
						<c:choose>
							<c:when test="${sessionScope.m_id==null}">
								<li class="nav-item"><a class="nav-link" href="memberLogin" id="nav-link">로그인</a>
								</li>
								<li class="nav-item"><a class="nav-link" href="memberJoin" id="nav-link">회원가입</a></li>
							</c:when>
							<c:otherwise>
								<li class="nav-item"><span class="nav-link" id="nav-link">${sessionScope.m_id}님 환영합니다.</span></li>
								<li class="nav-item"><a class="nav-link" href="memberLogout" id="nav-link">로그아웃</a></li>
							</c:otherwise>
						</c:choose>
						...
						<c:if test="${sessionScope.m_id!=null}">
							...
							<li class="nav-item"><a class="nav-link" id="nav-link" href="${pageContext.request.contextPath}/planL?id=${sessionScope.m_id}&active=${sessionScope.m_active_kind}">플래너</a>
							</li>
						</c:if>
