...
function boardDel(){
	...
	var b_Group = "${boardDetail.b_Group}";
	var b_Step = "${boardDetail.b_Step}";
// 	alert("b_kind->" + b_kind + ", b_no->" + b_no + ", m_id->" + m_id);
	Swal.fire({
        ...
        if (result.isConfirmed) {
        	alert("게시글이 삭제 되었습니다.");
        	location.href="HJBoardDelete?b_no="+b_no+"&m_id="+m_id+"&b_Group="+b_Group+"&b_Step="+b_Step;
        }
    });
}
...
//                      console.log("m_kind->" + m_kind);
					 // 비밀글O, 댓글 작성자 or 관리자                  	 
                   	 if(bc_lock == 'y' && (session_id == m_id || session_m_kind == '2')){
		                 ...
		                 a += '<a class="text-secondary" style="text-decoration: none;" onclick="commentDelete(' + bc_no + ', '+ bc_Group + ', ' + bc_Step +', \''+ bc_contents +'\');"> 삭제 </a>';
...
//엔터할 때 값이 없으면 입력하라고 안내문, 있으면 등록처리
document.body.addEventListener('keydown', (event) => { 
	const keyCode = event.keyCode; 
	if (keyCode === 13) {
// 		 alert("enter");
		if(document.activeElement.id == "bc_contents"){
// 			alert("contents");
		...
		}else if(document.activeElement.id == "c_modify"){
// 			alert("c_modify");
    		if(document.getElementById('c_modify').value == ""){
    			alert("댓글 수정 내용을 입력해주세요");
    		}else if(document.getElementById('c_modify').value != ""){
    		   var updateData = $("form[name=commentUpdateForm]").serialize();
//     		   alert("updateData->" + updateData);
    		   commentUpdateProc(updateData);
    		}
		}else if(document.activeElement.id == "bc_Replycontents"){
// 			alert("bc_Replycontents");
    		if(document.getElementById('bc_Replycontents').value == ""){
    			alert("댓글 답변 내용을 입력해주세요");
    		}else if(document.getElementById('bc_Replycontents').value != ""){
    		    var replyData = $("form[name=commentReplyForm]").serialize();
//     		    alert("replyData->" + replyData);
    		    commentReplyProc(replyData);
    		}
		}
	} 
});
...
// 	    alert("insertData->" + insertData); // insertData->b_no=230&b_kind=5&c_nickname=zinoxxl&c_lock=n&c_contents=hi
	    ...
	    $("input:checkbox[id='checkLock']").prop("checked", false);
	    $("#bc_lock").val("n");
	}
});
//댓글 등록
...
//     	alert("$('#bc_Replycontents').length < 0 - 대댓글 X");
	    if(bc_lock == 'y'){
	    	a += '<form name="commentUpdateForm" class="mt-3 md-3">';
		    a += '<div class="input-group">';
	    	a += '<input type="hidden" id="boardKind" name="b_kind" value="'+ b_kind +'"/>';
	    	a += '<input type="hidden" id="boardNo" name="b_no" value="'+ b_no +'"/>';
	    	a += '<input type="hidden" id="boardCommentNo" name="bc_no" value="'+ bc_no +'"/>';
		    a += '<input type="text" class="form-control" id="c_modify" name="bc_contents" value="'+bc_contents+'" required/>';
		    ...
		    a += '<button  style="background-color: #BBDFC8;" class="btn btn-outline-light text-white" type="button" name="commentUpdateBtn" onclick="updateForm();"> 수정</button></span>';
		    a += '</div>';
		    a += '</form>';
	    }else{
	    	a += '<form name="commentUpdateForm" class="mt-3 md-3">';
...
//댓글 수정
function commentUpdateProc(updateData){
    $.ajax({
...
// 댓글 삭제 
function commentDelete(bc_no, bc_Group, bc_Step, bc_contents){
// 	alert("bc_Group->" + bc_Group);
// 	alert("bc_Step->" + bc_Step);
	Swal.fire({
        title: '댓글 삭제',
        text: '\'' + bc_contents + "\' 댓글을 삭제하시겠습니까?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#FFC478',
        cancelButtonColor: '#F0E5D8',
...
//     	alert("$('#c_modify').length == 0 - 수정 X");
	    ...
	    a += '<input type="hidden" id="bc_kind" name="b_kind" value="${boardDetail.b_kind}"/>';
	    a += '<input type="hidden" id="b_no" name="b_no" value="${boardDetail.b_no}"/>';
	    a += '<input type="hidden" id="m_id" name="m_id" value="${sessionScope.m_id}">';
	    a += '<input type="hidden" id="bc_no" name="bc_no" value="'+ bc_no +'"/>';
	    a += '<input type="hidden" id="bc_Group" name="bc_Group" value="'+ bc_Group +'">';
	    a += '<input type="hidden" id="bc_Step" name="bc_Step" value="'+ bc_Step +'">';
	    a += '<input type="hidden" id="bc_Indent" name="bc_Indent" value="'+ bc_Indent +'">';
	    a += '<input type="text" class="form-control" id="bc_Replycontents" name="bc_contents" placeholder="내용을 입력하세요." required/>';
	    a += '<span class="input-group-btn">';
	    a += '<label><input type="checkbox" id="commentLock" name="commentLock" value=""> 비밀글 </label>';
	    a += '<input type="hidden" id="c_lock" name="bc_lock" value="">';
	    a += '<button style="background-color: #BBDFC8;" class="btn btn-outline-light text-white" type="button" onclick="replyform();"> 등록</button></span>';
	    a += '</div></form>';
...
//대댓글 달기
function commentReplyProc(replyData){
    $.ajax({
